...
export function useUploads() {
  ...
  function queueFiles(files: MediaFile[]) {
    let acceptedCount = 0
  ...
  async function startUpload(item: UploadItem) {
    ...
    try {
      ...
      if (pausedUploads.current.has(item.id)) return
  ...
  async function uploadChunks(item: UploadItem, session: UploadSession) {
    ...
    const chunkIndexes = Array.from({ length: session.totalChunks }, (_, i) => i).filter(
    ...
    async function worker() {
      while (nextIndex < chunkIndexes.length && !firstError) {
        if (cancelledUploads.current.has(item.id) || pausedUploads.current.has(item.id)) return
        ...
        try {
          const updatedSession = await uploadChunkWithRetry(item.id, session, chunkIndex, tempUri)
          const statusOverride = pausedUploads.current.has(item.id)
